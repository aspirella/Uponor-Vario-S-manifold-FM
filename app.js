...
function renderStep(index) {
    ...
    if (mainImage) {
        if (step.visual) {
            ...
            if (mainImage.src.split('?')[0] !== step.visual) {
...
function updateNavState(index) {
    ...
    const matchingNavItems = Array.from(navItems).filter(item => Number(item.dataset.step) === targetNavId);
...
function setTheme(mode) {
    ...
    // Synchronize all toggles and labels
...
function attachEventListeners() {
    ...
    mobileNavItems.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const stepIndex = stepsData.findIndex(s => String(s.id) === stepId);
            if (stepIndex !== -1) {
...
// GIF Pre-loading logic
function preloadNextStep() {
    const nextIndex = currentStepIndex + 1;
    if (nextIndex < stepsData.length && stepsData[nextIndex].visual) {
        const img = new Image();
        img.src = stepsData[nextIndex].visual;
    }
}

// Optimized trigger for pre-loading using RequestIdleCallback
if ('requestIdleCallback' in window) {
    const originalRenderStep = renderStep;
    window.renderStep = function (index) {
        originalRenderStep(index);
        requestIdleCallback(() => preloadNextStep());
    };
} else {
    const originalRenderStep = renderStep;
    window.renderStep = function (index) {
        originalRenderStep(index);
        setTimeout(preloadNextStep, 200);
    };
}
